...
| # | Check | Acción |
|---|---|---|
| 0 | **Valores de conjuro/mecánica** | Si la acción usa un conjuro → has consultado el MCP `conjuros_buscar_conjuro`. Si es un rasgo de clase → has consultado el MCP `clases_search_features`/`clases_get_feature_by_id`. Si es un dote → has consultado el MCP `dotes_detalle_dote`. Si es otra mecánica sin conjuro, rasgo ni dote → has delegado en Rules Keeper (nunca de memoria) |
